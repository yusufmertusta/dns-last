...
import { DNSLoadBalancerList } from "@/components/dnsLoadBalancer/DNSLoadBalancerList";
import { CreateDNSLoadBalancerDialog } from "@/components/dnsLoadBalancer/CreateDNSLoadBalancerDialog";
import { getDNSLoadBalancers, triggerDNSHealthCheck } from "@/lib/api";
...
export const DNSLoadBalancer = () => {
  ...
  const fetchLoadBalancers = async () => {
    try {
      const data = await getDNSLoadBalancers();
      setLoadBalancers(data);
    } catch (error: any) {
      toast({
        variant: "destructive",
        title: "Error",
        description: "Failed to fetch DNS load balancers: " + error.message,
      });
    } finally {
      setLoading(false);
    }
  ...
  const handleHealthCheck = async (loadBalancerId: string) => {
    try {
      setRefreshing(true);
      await triggerDNSHealthCheck(loadBalancerId);
      toast({
        title: "Success",
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Manage DNS load balancing for your domains
          </p>
        </div>
        ...
        <TabsContent value="overview" className="space-y-6">
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Common DNS load balancer operations
              </CardDescription>
            </CardHeader>
        ...
        <TabsContent value="health" className="space-y-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Real-time health status of all DNS servers
              </CardDescription>
            </CardHeader>
            <CardContent>
